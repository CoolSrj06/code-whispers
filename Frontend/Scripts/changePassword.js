...
form.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const response = await fetch('/api/v2/users/change-password', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            window.location.href = '/api/v2/url/?passwordUpdated=true'; // Redirect to homepage or dashboard URL with query parameter
